  ...
  return (
    <div
      style={{
        ...
        fontFamily: mono,
        // 0.72vw is the authored presentation size; the floor only engages
        // below ~1500px viewport, so the 16:9 render is untouched. Diegetic
        // text is garnish at phone scale — but it still has to be legible.
        fontSize: "max(0.72vw, 10px)",
    ...
    <Stage>
      ...
      <div
        style={{
          ...
          transform: "translateX(-50%)",
          fontFamily: mono,
          // floor + tightened tracking below the fold: at 0.55em the phone
          // string would overrun the frame once the floor lifts its size.
          fontSize: "max(1.05vw, 13px)",
          letterSpacing: "clamp(0.22em, 0.55vw, 0.55em)",
...
function DomainPhysicsVisual() {
  // The collision beat is the smallest thing on the slide and the one the
  // labels point at; at phone scale its shards drop to specks. Scale the
  // whole group as one unit so shard/ring geometry stays as authored.
  ...
  return (
    <Stage>
      ...
      {/* collision → growth — one rigid group so the compact scale can't
          shear the ring off the shards it belongs to */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          // The snapin keyframe flings the shards up by 40px, so at top:18%
          // there is no headroom on a phone — translate the whole group down
          // first, then scale about a center-left origin so it grows inward.
          transform: isMobile ? "translateY(9%) scale(1.45)" : undefined,
...
function SofaModeVisual() {
  // Phone frame is far narrower than 16:9, so a 9%-wide card grows into a
  // slab that gets sliced at both edges. Narrower cards keep the row reading
  // as slides drifting past rather than two clipped rectangles.
  const { isMobile } = useViewport();
  return (
...
function MontageVisual() {
  ...
  const cuts: { x: string; y: string; w: string; h: string; tint: string; d: number; motif: ReactNode }[] = [
    ...
    },
    {
      ...
      motif: (
        <div style={{ position: "absolute", left: "20%", top: "40%", fontFamily: mono, fontSize: "max(0.9vw, 11px)", color: palette.signal }}>
          &gt;&nbsp;
          <span style={{ display: "inline-block", width: "max(0.55vw, 6px)", height: "max(1.1vw, 12px)", background: palette.signal, verticalAlign: "text-bottom", animation: "tr-cursor 1.2s steps(1) infinite" }} />
        </div>
      ),
    },
    {
      ...
      motif: (
        <div style={{ position: "absolute", left: "14%", top: "16%", fontFamily: mono, fontSize: "max(0.85vw, 10px)", lineHeight: 1.5, color: `${palette.cloudViolet}bb`, letterSpacing: "0.2em" }}>
          {"@#%*\n*%@#\n#@*%"}
        </div>
      ),
    },
    ...
    <Stage bg="#060709">
      ...
      <div
        style={{
          ...
          transform: "translateX(-50%)",
          fontFamily: mono,
          // the deck's last line — floored so the end card still lands on a phone
