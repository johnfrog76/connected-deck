...
import { computeAtPodium, computeSofaActive, computeSuitcaseActive } from "./settingsRules";
...
const useStyles = makeStyles({
  ...
  modeCardDisabled: {
    cursor: "default",
    ":hover": {
      ...shorthands.borderColor(art.border),
    },
  },
  ...
  footerLink: { color: art.accent, textDecoration: "none" },
});
...
export function LaunchPage() {
  ...
  // Effective state, same rules as SettingsForm — imported from settingsRules,
  // not re-derived here, so the two surfaces can't drift the way they used to.
  // Podium needs a window wide enough for the notes popout, suitcase needs
  // sofa AND voice. The cards show where you actually ARE, not just what's
  // stored.
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.wrap}>
        ...
        {/* The proof section: this page is half the open-source pitch, and
            "trust us" isn't worth much when anyone can clone the repo. So the
            claim is checkable, in place, rather than asserted in the README. */}
        <section aria-label="Does it actually work?">
          <div className={styles.stageHead}>
            <h2 className={styles.stageTitle}>Does it actually work?</h2>
            <p className={styles.openSourceCopy}>Run the tests and see the results.</p>
          </div>
          <Checks />
        </section>
        ...
        <footer className={styles.footer}>
          ...
          <span>Slides are components. No backend needed to watch or hear a deck.</span>
        </footer>
      </div>
